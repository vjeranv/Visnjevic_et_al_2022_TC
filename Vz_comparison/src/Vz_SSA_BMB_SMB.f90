...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !
    Thickness       => VariableGet( Model % Mesh % Variables, 'Height',UnFoundFatal=UnFoundFatal)
    ...
    Vz              => VariableGet( Model % Mesh % Variables, 'Vz_RD',UnFoundFatal=UnFoundFatal)
    Vz_val          => Vz    % Values
    Vz_Perm         => Vz    % Perm!
    Vz_val          =  0.0_dp
...
DO e = 1, Solver % NumberOfActiveElements ! main loop through the active elements
 ...
 Do kk = 1,n  !! loop through the nodes of the element (e)
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Vz_val(Vz_Perm(NodeIndexes(kk))) = - ( SMB_val(SMB_Perm(NodeIndexes(kk)))*(rhoi/rhoo) - BMB_val(BMB_Perm(NodeIndexes(kk)))*(1.0_dp-rhoi/rhoo)) &
                                       - ( (Depth_val(Depth_Perm(NodeIndexes(kk))) + Thickness_val(Thickness_Perm(NodeIndexes(kk))) )*(1.0_dp - rhoi/rhoo)                                  &
